...
const ProfileButton: React.FC = () => {
  ...
  if (sessionData) {
                ...
                {({ active }) => (
                  <button
                    className={classNames(
                      ...
                      'block px-4 py-2 w-full text-sm text-neutral-700'
                    )}>
                    Your Profile
                  </button>
                )}
    ...
    return (
      <button
        className="text-neutral-300 hover:bg-neutral-700 hover:text-white px-3 py-2 rounded-md text-sm font-medium"
        onClick={() => signIn()}>
        Login
      </button>
    );
